...
public class SettingsTab extends JPanel {
    ...
    private JPanel buildMemoryPanel() {
        ...
        SpinnerNumberModel model = new SpinnerNumberModel(initRam, 512, maxRam, 256);
        ...
        warn.setForeground(new Color(0xC89060));
    ...
    private JPanel buildJvmPanel() {
        ...
        jvmArgsArea.setBackground(new Color(0x16140F));
        ...
        reset.addActionListener(e -> jvmArgsArea.setText(Settings.DEFAULT_JVM_ARGS));
    ...
    private JPanel buildJavaPanel() {
        ...
        javaDetectedLabel = new JLabel("Auto-detected: detecting...");
        javaDetectedLabel.setForeground(DIM);
        javaDetectedLabel.setFont(new Font("Segoe UI", Font.PLAIN, 11));

        // Detect Java path asynchronously — resolveJava() walks filesystem and must not run on EDT
        new SwingWorker<String, Void>() {
            @Override protected String doInBackground() { return GameLauncher.resolveJava(); }
            @Override protected void done() {
                try { javaDetectedLabel.setText("Auto-detected: " + get()); }
                catch (Exception ignored) { javaDetectedLabel.setText("Auto-detected: (detection failed)"); }
            }
        }.execute();
    ...
    private void saveSettings() {
        ...
        // Update Java label asynchronously after save
        javaDetectedLabel.setText("Auto-detected: detecting...");
        new SwingWorker<String, Void>() {
            @Override protected String doInBackground() { return GameLauncher.resolveJava(); }
            @Override protected void done() {
                try { javaDetectedLabel.setText("Auto-detected: " + get()); }
