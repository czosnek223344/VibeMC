...
public class FabricSetup {
    ...
    private static File installFabric(String loaderVersion, JsonObject mcProfile,
        ...
        // Skip if already installed with this exact version
        if (profileFile.exists()) {
            cb.update("Fabric " + loaderVersion + " already installed.", progEnd, 100);
            return profileFile;
        }
    ...
    private static void downloadLibraries(JsonArray libs, ProgressCallback cb,
                                           int progStart, int progEnd) throws Exception {
        int total = libs.size();
        int done  = 0;
        ...
        for (JsonElement el : libs) {
            ...
            if (lib.has("rules") && !rulesAllow(lib.getAsJsonArray("rules"))) {
                cb.update("Libraries " + done + "/" + total,
                    progStart + (int)((double) done / total * (progEnd - progStart)), 100);
                continue;
            }
            ...
            if (lib.has("name")) {
                ...
                // No outer exists() guard — downloadIfMissing handles existence + sha1 check,
                // so corrupted/truncated cached jars are detected and re-downloaded automatically.
                try {
                    if (lib.has("downloads")) {
                        ...
                        if (downloads.has("artifact")) {
                            ...
                            downloadIfMissing(artifactUrl, dest, sha1, cb, progStart, progEnd);
                        }
                    } else {
                        // Fabric-style library: has a repo base URL + optional sha1 at lib level
                        String base = lib.has("url") ? lib.get("url").getAsString() : null;
                        String artifactUrl = (base != null && !base.isBlank())
                            ? mavenUrlWithBase(name, base)
                            : mavenUrl(name);
                        String sha1 = lib.has("sha1") ? lib.get("sha1").getAsString() : null;
                        downloadIfMissing(artifactUrl, dest, sha1, cb, progStart, progEnd);
                    }
                } catch (Exception e) {
                    System.err.println("[Libs] Failed: " + name + ": " + e.getMessage());
                    failed.add(name);
                }
            }
        ...
        if (!failed.isEmpty()) {
            System.err.println("[Libs] " + failed.size() + " library download(s) failed:");
            failed.forEach(n -> System.err.println("  - " + n));
            // Non-fatal: game may still launch if missing libs are optional or already on classpath
        }
    }
    ...
    private static void downloadAssets(String indexId, String indexUrl,
        ...
        String indexJson  = Files.readString(indexFile.toPath(), java.nio.charset.StandardCharsets.UTF_8);
        ...
        int threads = (Settings.maxCpuThreads > 0) ? Settings.maxCpuThreads : 4;
    ...
    public static void downloadIfMissing(String url, File dest, String expectedSha1,
                                          ProgressCallback cb, int progStart, int progEnd) throws Exception {
        if (dest.exists()) {
            // Honor skipHashCheck setting — skip sha1 verification if user toggled it
            if (expectedSha1 == null || dev.vibemc.config.Settings.skipHashCheck) return;
            try {
                if (sha1(dest).equalsIgnoreCase(expectedSha1)) return;
            } catch (Exception e) {
                System.err.println("[Download] sha1 check failed for " + dest.getName() + ": " + e.getMessage());
                // Fall through and re-download
            }
            dest.delete(); // re-download if hash mismatch or unreadable
        }
        ...
        try {
            ...
            // renameTo can silently fail cross-filesystem (common on Windows) — fallback to Files.move
            if (!tmp.renameTo(dest)) {
                Files.move(tmp.toPath(), dest.toPath(), java.nio.file.StandardCopyOption.REPLACE_EXISTING);
            }
        } catch (Exception e) {
            tmp.delete(); // clean up partial or orphaned temp file on any failure
            throw e;
        }
    }
    ...
    private static String mavenUrlWithBase(String name, String base) {
        String[] parts    = name.split(":");
        String group      = parts[0].replace('.', '/');
        String artifact   = parts[1];
        String version    = parts[2];
    ...
    /** Public wrapper for update checks */
    public static String getLatestFabricLoaderPublic() throws Exception {
        return getLatestFabricLoader();
    }

    /** Returns the installed Fabric loader version string, or null if not installed */
    public static String getInstalledFabricVersion() {
        File profileFile = getInstalledProfileFile();
        if (profileFile == null) return null;
        // Profile dir name is "1.21.4-fabric-X.Y.Z"
        String dirName = profileFile.getParentFile().getName();
        String prefix = Main.GAME_VERSION + "-fabric-";
    ...
    public static File getInstalledProfileFile() {
        ...
        // Sort descending using semver-aware comparator so 0.16.10 > 0.16.9 (not string order)
        Arrays.sort(entries, (a, b) -> compareFabricVersionDesc(b.getName(), a.getName()));
        for (File f : entries) {
    ...
    /**
     * Compares two directory names like "1.21.4-fabric-0.16.9" and "1.21.4-fabric-0.16.10"
     * using numeric segment comparison so version ordering is correct.
     * Returns positive if a > b (for descending sort).
     */
    private static int compareFabricVersionDesc(String a, String b) {
        String prefix = Main.GAME_VERSION + "-fabric-";
        String va = a.startsWith(prefix) ? a.substring(prefix.length()) : a;
        String vb = b.startsWith(prefix) ? b.substring(prefix.length()) : b;
        String[] pa = va.split("[.+\\-]");
        String[] pb = vb.split("[.+\\-]");
