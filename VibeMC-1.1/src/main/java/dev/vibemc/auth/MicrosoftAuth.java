...
public class MicrosoftAuth {
    ...
    public static Account loadAccountFromDir(File playerDir) throws Exception {
        ...
        JsonObject root = JsonParser.parseString(
            Files.readString(mcaFile.toPath(), java.nio.charset.StandardCharsets.UTF_8).trim()).getAsJsonObject();
        ...
        if (liveFile != null) {
            try {
                JsonObject liveRoot = JsonParser.parseString(
                    Files.readString(liveFile.toPath(), java.nio.charset.StandardCharsets.UTF_8).trim()).getAsJsonObject();
    ...
    public static List<Account> importFromOfficialLauncher() throws Exception {
        ...
        String raw = Files.readString(accountsFile.toPath(), java.nio.charset.StandardCharsets.UTF_8);
    ...
    public static void saveToTokensFolder(String username, String accessToken,
        ...
        try {
            ...
            // Rename folder if MC name changed (same UUID, different folder name)
            if (uuid != null && !uuid.isEmpty()) {
                File[] existing = tokensDir.listFiles(File::isDirectory);
                if (existing != null) {
                    for (File dir : existing) {
                        if (dir.getName().equals(username)) continue; // already correct name, keep scanning
                        try {
                            File mcaFile = findCacheFile(dir, "mca");
                            if (mcaFile == null) continue;
                            JsonObject root = JsonParser.parseString(
                                Files.readString(mcaFile.toPath(), java.nio.charset.StandardCharsets.UTF_8).trim()).getAsJsonObject();
                            JsonObject mca = root.has("mca") ? root.getAsJsonObject("mca") : root;
                            String storedUuid = str(mca, "username", "");
                            // "username" field in mca-cache stores the UUID
                            String normalizedStored = storedUuid.replace("-", "");
                            String normalizedNew    = uuid.replace("-", "");
                            if (!normalizedStored.isEmpty() && normalizedStored.equals(normalizedNew)) {
                                File renamed = new File(tokensDir, username);
                                if (dir.renameTo(renamed)) {
                                    System.out.println("[Auth] Renamed token folder: "
                                        + dir.getName() + " → " + username);
                                } else {
                                    System.err.println("[Auth] Could not rename token folder "
                                        + dir.getName() + " to " + username);
                                }
                                break;
                            }
                        } catch (Exception ignored) {}
            ...
            File playerDir = new File(tokensDir, username);
    ...
    public static Account pollDeviceCode(DeviceCodeInfo info,
        ...
        // Mutable interval: MS device-code spec says increase by 5 s on every slow_down response
        long intervalMs = info.intervalSeconds * 1000L;
        ...
        while (System.currentTimeMillis() < deadline) {
            ...
            if (json.has("error")) {
                ...
                if ("slow_down".equals(error)) {
                    // Permanently increase the interval — future polls must also wait longer
                    intervalMs += 5000;
                    continue;
                }
    ...
    private static Account exchangeMsTokenToAccount(String msToken,
        ...
        String mcResp = post("https://api.minecraftservices.com/authentication/login_with_xbox",
            GSON.toJson(mcBody), "application/json");
