...
public class AccountManager {
    ...
    /** Upsert by UUID. Silently ignores accounts with null/empty UUID. */
    public static void add(Account a) {
        if (a.uuid == null || a.uuid.isEmpty()) return;
        accounts.removeIf(x -> a.uuid.equals(x.uuid));
        accounts.add(a);
        save();
    }
    ...
    /**
     * Atomically replace all accounts (used after token folder scan).
     * Filters out any entries with null/empty UUID. Saves exactly once.
     */
    public static void setAll(List<Account> newAccounts) {
        accounts.clear();
        for (Account a : newAccounts) {
            if (a.uuid != null && !a.uuid.isEmpty()) accounts.add(a);
        }
        save();
    }
    ...
    public static void load() {
        ...
        try {
            String json = Files.readString(Main.ACCOUNTS_FILE.toPath(), java.nio.charset.StandardCharsets.UTF_8);
    ...
    public static void save() {
        ...
        try {
            Files.writeString(Main.ACCOUNTS_FILE.toPath(),
                GSON.toJson(arr), java.nio.charset.StandardCharsets.UTF_8);
